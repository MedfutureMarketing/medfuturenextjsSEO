...
interface ApiResponse {
  podiatristJobs: BackendJob[]; // 👈 changed here
}
...
export default function RegistrarJobListingSection() {
  ...
  useEffect(() => {
    async function fetchJobs() {
      try {
        ...
        setJobs(res.podiatristJobs || []);
      } catch (error) {
        console.error("Failed to fetch podiatrist jobs", error);
      }
    }
  ...
  return (
    <section className="full-width-section lg:mt-[140px] lg:py-0 py-6 ">
      {/* Background decorations */}


      <div className="inner-width-section mx-auto px-3 xs:px-4 sm:px-6 lg:px-8 relative z-10">
        <div className=" px-0 lg:px-0 md:px-8">  {/* Header Section */}
          <div className="mb-8 xs:mb-10 sm:mb-12 md:mb-14 lg:mb-16">
            ...
            <div className="mb-4 xs:mb-5 sm:mb-6 md:mb-[22px]">
              ...
              <p className="text-[11px] xs:text-xs sm:text-sm md:text-[15px] lg:text-[16px] text-[#4A5565] max-w-2xl leading-relaxed">
                Explore verified podiatrist roles across healthcare settings, tailored to your skills and career aspirations.
              </p>
            </div>
          </div>
