...
interface ProfessionPageResponse {
  podiatristJobCounts: number; // 👈 changed
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center justify-center w-full">
        ...
        <p className="mt-1.5 xs:mt-2 sm:mt-2.5 md:mt-3 lg:mt-2 text-[11px] xs:text-xs sm:text-sm md:text-[14px] lg:text-[14px] text-slate-600 text-center leading-tight px-1 xs:px-2 max-w-full lg:max-w-none font-medium">
          {item.label}
        </p>
      </div>
    </div>
...
export default function RegistrarCounterSection() {
    ...
    async function fetchProfessionData() {
      try {
        ...
        const updatedCounters: CounterItem[] = [
          {
            value: res.podiatristJobCounts,
            label: "Active Specialist Podiatrist Jobs",
            suffix: "+",
          },
          {
            value: 9000,
            label: "Top-rated Podiatrists in Talent Pool",
            suffix: "+",
          },
          {
            value: 8,
  ...
  return (
    <section className="lg:mb-[140px] mb-[100px]"> <div className=" px-0 lg:px-0 md:px-8">
