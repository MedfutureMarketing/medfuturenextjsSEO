...
type MetadataResolver = MetadataConfig | (() => Promise<MetadataConfig>) | ((params: TemplateParams) => Promise<MetadataConfig>);

// Static + dynamic metadata
export const metaDataList: Record<string, MetadataResolver> = {
    home: async (): Promise<MetadataConfig> => {
    ...
    permanent: async (params: TemplateParams): Promise<MetadataConfig> => {
        const BASE_URL = await getBASEURL();
        return {
...
export const getDynamicOverrides = async (): Promise<Record<string, MetadataConfig>> => {
    const BASE_URL = await getBASEURL();
    return {
